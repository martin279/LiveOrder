unit ufrmCustomerOrderSYComponent;
...
type
  TfrmCustomerOrderSYComponent = class(TfrmDBBasicDT)
    ...
    adt_activeIsRebate2: TStringField;
...
procedure TfrmCustomerOrderSYComponent.SetData;
begin
  ...
  DM.DataSetQuery4(adt_active, 'EXEC usp_GetCustomerOrderComponent @CustomerOrderID = 1000000');
  DM.DataSetQuery4(adt_active2, 'EXEC usp_GetCustomerOrderComponentChangeLog @CustomerOrderID = 1000000');
end;

procedure TfrmCustomerOrderSYComponent.SetUI;
begin
  inherited;
...
procedure TfrmCustomerOrderSYComponent.SetAccess;
var
  i: integer;
begin
  inherited;
...
procedure TfrmCustomerOrderSYComponent.tbtnOperationClick(Sender: TObject);
begin
  //inherited;
...
procedure TfrmCustomerOrderSYComponent.dbcbbPlantChange(Sender: TObject);
begin
...
procedure TfrmCustomerOrderSYComponent.MenuAddOrderCommentsClick(Sender: TObject);
begin
...
procedure TfrmCustomerOrderSYComponent.btnQueryClick(Sender: TObject);
var
...
{  if chkFY.Checked then
  ...
  sqlConst := 'EXEC usp_GetCustomerOrderComponent ' + sqlConst + pRoles + PStates + ',@Col=''ContractDate desc''';
  try
...
procedure TfrmCustomerOrderSYComponent.actGridSTFilterExecute(Sender: TObject);
begin
  //inherited;
...
procedure TfrmCustomerOrderSYComponent.gridDataCellClick(Column: TColumnEh);
begin
  inherited;
  if gridData.DataSource.DataSet.RecordCount > 0 then
  begin
    GroupBox2.Caption := 'Change Log Of Customer Order ID : ' + gridData.DataSource.DataSet.fieldbyname('CustomerOrderID').AsString;
    DM.DataSetQuery4(adt_active2, 'EXEC usp_GetCustomerOrderComponentChangeLog @CustomerOrderID=' + gridData.DataSource.DataSet.fieldbyname('CustomerOrderID').AsString);
  end;
end;
