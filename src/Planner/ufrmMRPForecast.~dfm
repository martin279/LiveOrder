inherited frmMRPForecast: TfrmMRPForecast
  Caption = 'frmMRPForecast'
  PixelsPerInch = 96
  TextHeight = 13
  inherited PnlTop: TPanel
    inherited CoolBar1: TCoolBar
      inherited ToolBar1: TToolBar
        inherited DBNavigator1: TDBNavigator
          Hints.Strings = ()
        end
      end
    end
  end
  inherited pnlBottom: TPanel
    object btnGenerateMRP: TButton
      Left = 384
      Top = 4
      Width = 81
      Height = 25
      Caption = 'Generate MRP'
      TabOrder = 0
      OnClick = btnGenerateMRPClick
    end
    object ProgressBar1: TProgressBar
      Left = 16
      Top = 8
      Width = 353
      Height = 17
      TabOrder = 1
    end
  end
  inherited pnlBody: TPanel
    inherited PageControl1: TPageControl
      inherited tbsBody1: TTabSheet
        inherited GroupBox1: TGroupBox
          inherited gridData: TDBGridEh
            Columns = <
              item
                EditButtons = <>
                FieldName = 'ForecastID'
                Footers = <>
                Title.Caption = 'ID'
              end
              item
                EditButtons = <>
                FieldName = 'ComponentItem'
                Footers = <>
              end
              item
                EditButtons = <>
                FieldName = 'Quantity'
                Footers = <>
              end
              item
                EditButtons = <>
                FieldName = 'ForecastMonth'
                Footers = <>
                Width = 125
              end
              item
                EditButtons = <>
                FieldName = 'ForecastYear'
                Footers = <>
                Width = 85
              end>
          end
        end
      end
    end
  end
  inherited PrintDBGridEh1: TPrintDBGridEh
    BeforeGridText_Data = {
      
      66315C66733136200D0A5C706172207D0D0A00}
    AfterGridText_Data = {
      
      706172207D0D0A00}
  end
  inherited adt_active: TADODataSet
    Connection = DM.ADOConnection1
    CommandText = 
      'select s.*,c.ComponentItem from SCMRPForecast s'#13#10'left outer join' +
      ' Component c on s.ComponentID=c.ComponentID'
    object adt_activeForecastID: TAutoIncField
      FieldName = 'ForecastID'
      ReadOnly = True
    end
    object adt_activeComponentID: TIntegerField
      FieldName = 'ComponentID'
    end
    object adt_activeQuantity: TIntegerField
      FieldName = 'Quantity'
    end
    object adt_activeForecastMonth: TIntegerField
      FieldName = 'ForecastMonth'
    end
    object adt_activeForecastYear: TIntegerField
      FieldName = 'ForecastYear'
    end
    object adt_activeComponentItem: TStringField
      FieldName = 'ComponentItem'
      Size = 30
    end
  end
end
