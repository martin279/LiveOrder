...
type
  TfrmPurchaseOrderJIT = class(TfrmDBBasicTree)
...
procedure TfrmPurchaseOrderJIT.SetData;
begin
  //DM.DataSetQuery(adt_active, format(sQueryPurchaseOrderSQL, ['']));
  DM.DataSetQuery(adt_active, 'EXEC usp_GetPurchaseOrderJIT');
end;

procedure TfrmPurchaseOrderJIT.SetUI;
begin
  inherited;
...
procedure TfrmPurchaseOrderJIT.SetAccess;
begin
  inherited;
...
procedure TfrmPurchaseOrderJIT.LoadTreeView(nodeName: string);
var
  ftn_type: TTreeNode;
    ...
    case cbbTreeType.ItemIndex of
      ...
      1:
        begin
          ...
          sqlConst := 'select convert(varchar(16),ETD,120) as ETD,count(*) as orderCount from PurchaseOrderJIT where 0=0';
...
procedure TfrmPurchaseOrderJIT.TreeView1Change(Sender: TObject;
  Node: TTreeNode);
var
  sqlConst: string;
  i: integer;
begin
  ...
  DM.DataSetQuery(adt_active, 'EXEC usp_GetPurchaseOrderJIT ' + sqlConst);
end;

procedure TfrmPurchaseOrderJIT.gridDataGetCellParams(Sender: TObject;
...
procedure TfrmPurchaseOrderJIT.adt_activeBeforePost(DataSet: TDataSet);
begin
  try
...
procedure TfrmPurchaseOrderJIT.MenuGeneratePickUpFormClick(Sender: TObject);
var
  oTable: WordXP.Table;
  a, b, c: OleVariant;
