inherited frmProductPackgeCUD: TfrmProductPackgeCUD
  Caption = 'frmProductPackgeCUD'
  PixelsPerInch = 96
  TextHeight = 13
  inherited PnlTop: TPanel
    inherited CoolBar1: TCoolBar
      inherited ToolBar1: TToolBar
        inherited DBNavigator1: TDBNavigator
          Hints.Strings = ()
        end
      end
    end
  end
  inherited pnlBody: TPanel
    inherited PageControl1: TPageControl
      inherited tbsBody1: TTabSheet
        inherited GroupBox1: TGroupBox
          inherited gridData: TDBGridEh
            Columns = <
              item
                EditButtons = <>
                FieldName = 'ProductPackgeID'
                Footers = <>
                Title.Caption = 'Product Packge ID'
              end
              item
                EditButtons = <>
                FieldName = 'IsCHN'
                Footers = <>
                Title.Caption = 'Is CHN'
                Width = 34
              end
              item
                EditButtons = <>
                FieldName = 'ProductSeriesName2'
                Footers = <>
                Title.Caption = 'Product Series Name'
              end
              item
                EditButtons = <>
                FieldName = 'ProductPackgeType'
                Footers = <>
                Title.Caption = 'Product Packge Type'
                Width = 77
              end
              item
                EditButtons = <>
                FieldName = 'ProductWeightUnit'
                Footers = <>
                Title.Caption = 'Product WeightUnit'
                Width = 68
              end
              item
                EditButtons = <>
                FieldName = 'ProductWeight'
                Footers = <>
                Title.Caption = 'Product Weight'
              end
              item
                EditButtons = <>
                FieldName = 'ProductGrossWeight'
                Footers = <>
                Title.Caption = 'Product Gross Weight'
                Width = 104
              end
              item
                EditButtons = <>
                FieldName = 'ProductSizeUnit'
                Footers = <>
                Title.Caption = 'Product Size Unit'
                Width = 57
              end
              item
                EditButtons = <>
                FieldName = 'ProductSize'
                Footers = <>
                Title.Caption = 'Product Size'
              end>
          end
        end
      end
    end
  end
  inherited PrintDBGridEh1: TPrintDBGridEh
    BeforeGridText_Data = {
      
      66315C66733136200D0A5C706172207D0D0A00}
    AfterGridText_Data = {
      
      706172207D0D0A00}
  end
  inherited adt_active: TADODataSet
    CommandText = 
      'select p.*,s.ProductSeriesName'#13#10'     from ProductPackgeCUD p'#13#10'  ' +
      '   left outer join ProductSeries s on p.ProductSeriesID=s.Produc' +
      'tSeriesID'
    object adt_activeProductPackgeID: TIntegerField
      FieldName = 'ProductPackgeID'
    end
    object adt_activeIsCHN: TStringField
      FieldName = 'IsCHN'
      Size = 1
    end
    object adt_activeProductSeriesID: TIntegerField
      FieldName = 'ProductSeriesID'
    end
    object adt_activeProductPackgeType: TStringField
      FieldName = 'ProductPackgeType'
      Size = 10
    end
    object adt_activeProductWeightUnit: TStringField
      FieldName = 'ProductWeightUnit'
      Size = 5
    end
    object adt_activeProductWeight: TFloatField
      FieldName = 'ProductWeight'
    end
    object adt_activeProductGrossWeight: TFloatField
      FieldName = 'ProductGrossWeight'
    end
    object adt_activeProductSizeUnit: TStringField
      FieldName = 'ProductSizeUnit'
      Size = 5
    end
    object adt_activeProductSize: TStringField
      FieldName = 'ProductSize'
    end
    object adt_activeProductSeriesName: TStringField
      FieldName = 'ProductSeriesName'
      Size = 15
    end
    object adt_activeProductSeriesName2: TStringField
      FieldKind = fkLookup
      FieldName = 'ProductSeriesName2'
      LookupDataSet = adt_pt
      LookupKeyFields = 'ProductSeriesID'
      LookupResultField = 'ProductSeriesName'
      KeyFields = 'ProductSeriesID'
      Size = 30
      Lookup = True
    end
  end
  object adt_pt: TADODataSet
    Connection = DM.ADOConnection1
    CommandText = 'select * from ProductSeries'
    Parameters = <>
    Left = 209
    Top = 8
  end
end
