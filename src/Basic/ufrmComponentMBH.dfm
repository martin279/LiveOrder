inherited frmComponentMBH: TfrmComponentMBH
  Left = 432
  Top = 242
  Caption = 'frmComponentMBH'
  PixelsPerInch = 96
  TextHeight = 13
  inherited PnlTop: TPanel
    inherited CoolBar1: TCoolBar
      inherited ToolBar1: TToolBar
        inherited DBNavigator1: TDBNavigator
          Hints.Strings = ()
        end
      end
    end
  end
  inherited pnlBody: TPanel
    inherited PageControl1: TPageControl
      inherited tbsBody1: TTabSheet
        inherited GroupBox1: TGroupBox
          inherited gridData: TDBGridEh
            Columns = <
              item
                EditButtons = <>
                FieldName = 'ComponentMBHID'
                Footers = <>
                Width = 108
              end
              item
                EditButtons = <>
                FieldName = 'ComponentItem'
                Footers = <>
                Width = 105
              end
              item
                EditButtons = <>
                FieldName = 'CurrentInventoryCount'
                Footers = <>
                Width = 124
              end
              item
                EditButtons = <>
                FieldName = 'CurrentInventoryDate'
                Footers = <>
                Width = 123
              end
              item
                EditButtons = <>
                FieldName = 'MinimumLevel'
                Footers = <>
                Width = 85
              end
              item
                EditButtons = <>
                FieldName = 'MaximumLevel'
                Footers = <>
                Width = 79
              end>
          end
        end
      end
    end
  end
  inherited PrintDBGridEh1: TPrintDBGridEh
    BeforeGridText_Data = {
      
      66315C66733136200D0A5C706172207D0D0A00}
    AfterGridText_Data = {
      
      706172207D0D0A00}
  end
  inherited adt_active: TADODataSet
    CursorType = ctStatic
    object adt_activeComponentMBHID: TAutoIncField
      FieldName = 'ComponentMBHID'
      ReadOnly = True
    end
    object adt_activeComponentItem: TStringField
      FieldName = 'ComponentItem'
      Size = 50
    end
    object adt_activeCurrentInventoryCount: TBCDField
      FieldName = 'CurrentInventoryCount'
      Precision = 18
      Size = 0
    end
    object adt_activeCurrentInventoryDate: TDateTimeField
      FieldName = 'CurrentInventoryDate'
    end
    object adt_activeMinimumLevel: TBCDField
      FieldName = 'MinimumLevel'
      Precision = 18
      Size = 0
    end
    object adt_activeMaximumLevel: TBCDField
      FieldName = 'MaximumLevel'
      Precision = 18
      Size = 0
    end
  end
end
