inherited frmProductionSchedule: TfrmProductionSchedule
  Caption = 'frmProductionSchedule'
  PixelsPerInch = 96
  TextHeight = 13
  inherited PnlTop: TPanel
    inherited CoolBar1: TCoolBar
      inherited ToolBar1: TToolBar
        inherited DBNavigator1: TDBNavigator
          Hints.Strings = ()
        end
      end
    end
  end
  inherited pnlBottom: TPanel
    object btnImportProductionSchedule: TButton
      Left = 48
      Top = 8
      Width = 89
      Height = 25
      Caption = 'Import Schedule'
      TabOrder = 0
      OnClick = btnImportProductionScheduleClick
    end
  end
  inherited pnlBody: TPanel
    inherited PageControl1: TPageControl
      Top = 23
      Height = 357
      inherited tbsBody1: TTabSheet
        inherited GroupBox1: TGroupBox
          Height = 326
          inherited PreviewBox1: TPreviewBox
            Top = 235
          end
          inherited gridData: TDBGridEh
            Height = 179
            Columns = <
              item
                EditButtons = <>
                FieldName = 'ProductionScheduleID'
                Footers = <>
                Title.Caption = 'ID'
                Width = 55
              end
              item
                EditButtons = <>
                FieldName = 'Line1'
                Footers = <>
                Title.Caption = 'Line'
              end
              item
                EditButtons = <>
                FieldName = 'Model1'
                Footers = <>
                Title.Caption = 'Model'
              end
              item
                EditButtons = <>
                FieldName = 'ScheduleQuantity'
                Footer.ValueType = fvtSum
                Footers = <>
                Title.Caption = 'Schedule Qty.'
                Width = 95
              end
              item
                EditButtons = <>
                FieldName = 'RTD'
                Footers = <>
              end
              item
                EditButtons = <>
                FieldName = 'ScheduleStartTime'
                Footers = <>
              end
              item
                EditButtons = <>
                FieldName = 'AssemblyLineID'
                Footers = <>
                Visible = False
              end
              item
                EditButtons = <>
                FieldName = 'ModelID'
                Footers = <>
                Visible = False
              end
              item
                EditButtons = <>
                FieldName = 'Line'
                Footers = <>
                Visible = False
              end
              item
                EditButtons = <>
                FieldName = 'Model'
                Footers = <>
                Visible = False
              end>
          end
        end
      end
    end
    object Panel1: TPanel
      Left = 0
      Top = 0
      Width = 683
      Height = 23
      Align = alTop
      BevelInner = bvLowered
      BevelOuter = bvLowered
      Color = clSilver
      TabOrder = 1
      object Label1: TLabel
        Left = 2
        Top = 2
        Width = 679
        Height = 19
        Align = alClient
        Caption = #27880#65306#23548#20837#30340#29983#20135#35745#21010#21487#29992#20110#35745#31639'PO'#30340#26102#38388#33539#22260' : '#24403#22825'+30'#22825'>ScheduleStartTime>'#24403#22825#65293'3'#22825
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
    end
  end
  inherited PrintDBGridEh1: TPrintDBGridEh
    Left = 456
    Top = 21
    BeforeGridText_Data = {
      
      66315C66733136200D0A5C706172207D0D0A00}
    AfterGridText_Data = {
      
      706172207D0D0A00}
  end
  inherited adt_active: TADODataSet
    Top = 16
    object adt_activeProductionScheduleID: TAutoIncField
      FieldName = 'ProductionScheduleID'
      ReadOnly = True
    end
    object adt_activeLine: TStringField
      FieldName = 'Line'
      Size = 6
    end
    object adt_activeModel: TStringField
      FieldName = 'Model'
      Size = 25
    end
    object adt_activeScheduleQuantity: TIntegerField
      FieldName = 'ScheduleQuantity'
    end
    object adt_activeRTD: TDateTimeField
      FieldName = 'RTD'
    end
    object adt_activeScheduleStartTime: TDateTimeField
      FieldName = 'ScheduleStartTime'
    end
    object adt_activeAssemblyLineID: TIntegerField
      FieldName = 'AssemblyLineID'
    end
    object adt_activeModelID: TIntegerField
      FieldName = 'ModelID'
    end
    object adt_activeLine1: TStringField
      FieldKind = fkLookup
      FieldName = 'Line1'
      LookupDataSet = adt_line
      LookupKeyFields = 'AssemblyLineID'
      LookupResultField = 'Line'
      KeyFields = 'AssemblyLineID'
      Size = 10
      Lookup = True
    end
    object adt_activeModel1: TStringField
      FieldKind = fkLookup
      FieldName = 'Model1'
      LookupDataSet = adt_model
      LookupKeyFields = 'ModelID'
      LookupResultField = 'Model'
      KeyFields = 'ModelID'
      Size = 30
      Lookup = True
    end
    object adt_activePlantID: TIntegerField
      FieldName = 'PlantID'
    end
    object adt_activePlantCode: TStringField
      FieldName = 'PlantCode'
      Size = 2
    end
  end
  object adt_line: TADODataSet
    Connection = DM.ADOConnection1
    CommandText = 'select * from AssemblyLine'
    Parameters = <>
    Left = 210
    Top = 19
  end
  object adt_model: TADODataSet
    Connection = DM.ADOConnection1
    CommandText = 'select * from Model'
    Parameters = <>
    Left = 282
    Top = 17
  end
end
