inherited frmWIP: TfrmWIP
  Caption = 'frmWIP'
  PixelsPerInch = 96
  TextHeight = 13
  inherited PnlTop: TPanel
    inherited CoolBar1: TCoolBar
      inherited ToolBar1: TToolBar
        inherited DBNavigator1: TDBNavigator
          Hints.Strings = ()
        end
      end
    end
  end
  inherited pnlBody: TPanel
    inherited GroupBox1: TGroupBox
      inherited gridData: TDBGridEh
        Columns = <
          item
            EditButtons = <>
            FieldName = 'collect_id'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'line_num'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'model_code'
            Footers = <>
            Width = 77
          end
          item
            EditButtons = <>
            FieldName = 'wip_qty'
            Footers = <>
          end
          item
            EditButtons = <>
            FieldName = 'collect_time'
            Footers = <>
          end>
      end
    end
  end
  inherited PrintDBGridEh1: TPrintDBGridEh
    BeforeGridText_Data = {
      
      66315C66733136200D0A5C706172207D0D0A00}
    AfterGridText_Data = {
      
      706172207D0D0A00}
  end
  inherited adt_active: TADODataSet
    object adt_activecollect_id: TAutoIncField
      FieldName = 'collect_id'
      ReadOnly = True
    end
    object adt_activeline_num: TIntegerField
      FieldName = 'line_num'
    end
    object adt_activemodel_code: TStringField
      FieldName = 'model_code'
      FixedChar = True
      Size = 3
    end
    object adt_activewip_qty: TIntegerField
      FieldName = 'wip_qty'
    end
    object adt_activecollect_time: TDateTimeField
      FieldName = 'collect_time'
    end
  end
end
